## This is a differential expression analysis of breast cancer tumour samples to compare gene expression patterns before and after de NAC (NeoAdyuvant Chemotherapy).
...
# In this case we select pre NAC samples and their correspondent RCC files to build an RCCSet and get the counts matrix
...
write_csv(dea.o, "Results/NAC/NAC-DEA.csv")
...
pdf(file = 'Results/NAC/volcano_plot.pdf')
par(mar = c(5, 4, 4, 4))
...
write.csv(tab2[which(signGenes),], file = "Results/NAC/signGenes.csv")
...
write_csv(x = gsea_summary, path = "Results/NAC/NACstatus_gsea.csv")
